...
taus_1=np.array([ufloat(np.mean(tau_1_1),np.std(tau_1_1)),ufloat(np.mean(tau_1_2),np.std(tau_1_2)),ufloat(np.mean(tau_1_3),np.std(tau_1_3))])
...
ax.text(0.95,0.1,rf'<$\tau$> = {tau2:.2ue} s',bbox=dict(alpha=0.8),transform=ax.transAxes,ha='right', va='bottom')
plt.legend()
plt.grid()
plt.ylabel(r'$\tau$ (s)')
plt.xlabel('Indx')
...
taus_3=np.array([ufloat(np.mean(tau_3_1),np.std(tau_3_1)),ufloat(np.mean(tau_3_2),np.std(tau_3_2)),ufloat(np.mean(tau_3_3),np.std(tau_3_3))])
...
ax.text(0.95,0.1,rf'<$\tau$> = {tau3:.2ue} s',bbox=dict(alpha=0.8),transform=ax.transAxes,ha='right', va='bottom')
plt.legend()
plt.grid()
plt.ylabel(r'$\tau$ (s)')
plt.xlabel('Indx')
...
taus_4=np.array([ufloat(np.mean(tau_4_1),np.std(tau_4_1)),ufloat(np.mean(tau_4_2),np.std(tau_4_2)),ufloat(np.mean(tau_4_3),np.std(tau_4_3))])
...
for ax in [ax1,ax2,ax3,ax4]:
    ...
    ax.legend()
    ax.grid()
plt.suptitle('Ni=0 @citrato - 3ra repeticion',fontsize=14)
